    ...
    public function edit(PreOrder $preOrder): Response
    {
        ...
        if ($canEditAll) {
            ...
            if ($canEditWalkin) {
                ...
                // If they can only edit own, we still need to check if they have walk-in permission generally?
                // Actually the user said "same for edit", so granular should apply to edit as well.
                // If they only have 'edit own' + 'create walkin' then they should probably be able to edit their own walkin.
                // But for simplicity, let's stick to the granular ones being the primary controllers.
                $canEdit = true;
            }
        } else { // Regular order
            if ($canEditRegular) {
        ...
        try {
            ...
            if ($statusChangedToPaid) {
                ...
                if (!SmsSettings::isActive()) {
                    ...
                    Log::warning('SMS notification could not be sent because SMS service is currently deactivated.');
                } else {
                    try {
                        // Reload relationships for SMS
                        $preOrder->load(['collectionDay', 'collectionBranch', 'items.product']);
