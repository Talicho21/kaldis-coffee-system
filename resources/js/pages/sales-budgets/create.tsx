...
const breadcrumbs: BreadcrumbItem[] = [
	{ title: 'Sales Budgets', href: '/sales-budgets' },
	{ title: 'Add New Budget', href: '/sales-budgets/create' },
];
			...
			branches.map(async (branch) => {
				try {
					const [prevRes, checkRes] = await Promise.all([
						fetch(`/sales-budgets/prev-expense?fiscal_month_id=${fiscalMonthId}&year=${year}&branch_id=${branch.id}`),
						fetch(`/sales-budgets/check?branch_id=${branch.id}&fiscal_year_id=${selectedFiscalYearId}&fiscal_month_id=${fiscalMonthId}`),
					]);
				...
				<div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-3">
							<button
								onClick={() => router.get('/sales-budgets')}
