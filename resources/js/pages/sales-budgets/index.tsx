...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Sales Budgets', href: '/sales-budgets' }];
...
export default function SalesBudgetIndex({ budgets, branches, fiscalYears, fiscalMonths, canModify, request }: Props) {
	...
	function handleFilterSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		router.get('/sales-budgets', params, { preserveState: true, replace: true });
	}
	...
	function handleResetFilters() {
		...
		router.get('/sales-budgets', {}, { preserveState: true, replace: true });
	}
	...
	function confirmDelete() {
		...
		setDeleting(true);
		router.delete(`/sales-budgets/${deleteModal.id}`, {
			onSuccess: () => {
				closeDeleteModal();
				setDeleting(false);
			},
											...
											<td className="px-4 py-3">
												{budget.has_budget ? (
													<div className="flex items-center gap-2">
														<Link
															href={`/sales-budgets/${budget.id}/edit`}
